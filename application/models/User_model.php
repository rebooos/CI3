...
class User_model extends CI_Emerald_Model {
     ...
     * @return int
     */
    public function get_count_likes(): int
    {
     ...
     * @return bool
     */
    public function set_count_likes(int $count_likes)
    {
    	$this->count_likes = $count_likes;
    	return $this->save('count_likes', $count_likes);
    }
    ...
    private static function _preparation_main_page($data)
    {
	    ...
	    $o->count_likes = $data->get_count_likes();
    ...
    public static function add_money($sum)
    {
	    $user = User_model::get_user();

		App::get_ci()->s->from('history_balance')
			->insert([
				'user_id' => $user->id,
				'old_balance' => $user->wallet_balance,
				'new_balance' => $user->wallet_balance + $sum,
			])
	        ->execute();

	    return App::get_ci()->s
		    ->from(self::CLASS_TABLE)->where(['id' => $user->id])
		    ->update([
		    	'wallet_balance' => $user->wallet_balance + $sum,
			    'wallet_total_refilled' => $user->wallet_total_refilled + $sum,
			    ])
		    ->execute();
    }

    public function spend_money($sum) {

	    $wallet_balance = $this->wallet_balance - $sum;
	    if ($wallet_balance < 0) {
	    	return false;
	    }
	    ...
	    $this->wallet_balance = $wallet_balance;
	    return App::get_ci()->s
		    ->from(self::CLASS_TABLE)->where(['id' => $this->id])
		    ->update([
			    'wallet_balance' => $wallet_balance,
			    'wallet_total_withdrawn' => $this->wallet_total_withdrawn + $sum,
		    ])
		    ->execute();
    }

    public function spend_likes(int $type_id, int $type)
    {
    	$this->count_likes--;
		$this->set_count_likes($this->count_likes);

	    App::get_ci()->s->from('history_likes')
		    ->insert([
			    'user_id' => $this->id,
			    'type_id' => $type_id,
			    'type' => $type,
