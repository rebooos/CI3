...
class Comment_model extends CI_Emerald_Model
{
    ...
    public function set_children(Comment_model $comment)
    {
    	$this->children[$comment->get_id()] = $comment;
    }
    ...
    public static function get_tree(int $assting_id)
    {
	    ...
	    foreach ($data as $i)
	    {
	    	...
	    	if ($i['parent_id'] != 0) {
			    $ret[$i['parent_id']]->set_children($comment);
		    } else {
	    		$ret[$i['id']] = $comment;
		    }
	    }
    ...
    private static function _preparation_full_info($data)
    {
        ...
        foreach ($data as $d){
            ...
            $o->likes = $d->get_likes() ?? 0;
            $o->children = $d->get_children();
